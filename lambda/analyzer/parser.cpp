...
#else
			std::string error = "杂鱼\u2661~，是不是忘记添加表达式分隔符\";\"了~";
#endif
			input_croak(error);
			break;
		}
...
#else
			std::string error = "真笨\u2661~，是不是忘了另\u2661一\u2661半\u2661了\u2661~";
#endif
			input_croak(error);
			break;
		}
...
static void skip_op(const char* op)
{
	...
	{
		std::string error = "预期接受操作符：" + *op + '\"';
		input_croak(error);
	}
}
static Token* unexpected_token()
{
	std::string error = "未定义的Token";
	input_croak(error);
	return NULL;
}
...
#else
		std::string error = "{ } 并未包含任何表达式！你连自己想对我干什么都不知道嘛\u2661~";
#endif
		input_croak(error);
		// return NULL;
	}
